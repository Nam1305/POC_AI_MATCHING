...
this order: Job Title, Work Type, Location, Education, Skills, Experience
Requirement, Responsibilities, Requirements, Benefits. The first six are
structured values chosen from a picker/enum on the job-posting form — trust
them over anything inferred from the free-text Responsibilities/
Requirements/Benefits blocks that follow:
  ...
  - "Location: X" → the city name is the LAST comma-separated segment of
    this line; read city from there instead of guessing from prose.
  - "Education: X[, Y, ...]" → see EDUCATION rules below.
  - "Skills: X, Y, Z" → these are employer-picked tags. Always include every
    one of them in required_skills (weight 3) unless a Requirements bullet
    explicitly demotes that exact skill to preferred (see REQUIRED vs
    PREFERRED). Then also scan the Responsibilities/Requirements prose for
    ADDITIONAL skills not already covered by this list and merge them in
    (dedupe by canonical name) — the tag list and the prose are two
    complementary sources, never rely on only one.
...
block commonly mixes mandatory items with individually-flagged nice-to-haves
...
- preferred_skills = skill mentions carrying a softening cue anywhere in
  their own sentence/bullet: "Preferred", "Nice to have", "Plus", "Bonus",
  "Desirable", "advantage" / "a plus", "ưu tiên", "ưu tiên nếu có". Put them
  ONLY in preferred_skills, NEVER in required_skills, even when the bullet
  lives inside a "Requirements:" heading.
...
preferred_skills — it describes compensation/allowance tiers, not hiring
requirements, even when it mentions skill-like terms (e.g. a "N1"/"N2"
allowance amount does not mean N1/N2 is required or preferred if only N3
was actually asked for in Requirements).
